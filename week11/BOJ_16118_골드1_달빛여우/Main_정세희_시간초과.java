...
public class Main_정세희_시간초과 {
    ...
    private static int[] dijkstraFox(List<List<Node>> graph) {
        ...
        Queue<Node> pq = new PriorityQueue<>(Comparator.comparing(node -> node.cost));
        pq.add(new Node(1, 0));
        dist[1] = 0;
    ...
    private static int[] dijkstraWolf(List<List<Node>> graph) {
        ...
        Queue<Wolf> pq = new PriorityQueue<>(Comparator.comparingInt(o -> o.cost));
